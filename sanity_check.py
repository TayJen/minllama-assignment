...
with torch.no_grad():
    ...
    # print(sanity_data["logits"], sanity_data["logits"].shape)

    assert torch.allclose(logits, sanity_data["logits"], atol=1e-4, rtol=1e-3)
    assert torch.allclose(hidden_states, sanity_data["hidden_states"], atol=1e-4, rtol=1e-3)
